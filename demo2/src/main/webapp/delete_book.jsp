...
<body>
    ...
    <div class="container">
        <h2>删除图书信息</h2>
        <form action="delete_book.jsp" method="post">
            <div class="form-item">
                <label>书号：</label>
            ...
            <% 
                ...
                if (bookId != null && !bookId.isEmpty()) {
                    try {
                        ...
                        java.sql.PreparedStatement ps = conn.prepareStatement("DELETE FROM book WHERE bookid = ?");
                        ps.setString(1, bookId);
                        
                        int rows = ps.executeUpdate();
                        if (rows > 0) {
                            out.println("<p style='color: green;'>删除成功！</p>");
                        } else {
                            out.println("<p style='color: red;'>未找到该图书信息，删除失败！</p>");
                        }
                        ...
                        e.printStackTrace();
                        out.println("<p style='color: red;'>删除失败：" + e.getMessage() + "</p>");
                    }
                }
            %>
        </div>
